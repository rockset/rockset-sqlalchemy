...
def test_get_columns_with_describe_returns_expected_structure(engine, columns_from_describe_results):
    with mock.patch(
        "rockset_sqlalchemy.cursor.Cursor.execute_query", mock.Mock()
    ) as _, mock.patch(
        "rockset_sqlalchemy.sqlalchemy.dialect.RocksetDialect._validate_query", mock.Mock()
    ) as _, mock.patch(
        "rockset_sqlalchemy.sqlalchemy.dialect.RocksetDialect._exec_query",
        mock.Mock(return_value=columns_from_describe_results)
    ) as _:
        expected_results = [
            {"name": "_event_time", "type": Timestamp, "nullable": True, "default": None},
...
def test_get_columns_falls_back_on_rockset_exception_to_select_one_row(engine, columns_from_select_row_results):
    with mock.patch(
        ...
        expected_results = [
            ...
            {"name": "_meta", "type": NullType, "nullable": True, "default": None, },
            {"name": "name", "type": String, "nullable": True, "default": None},
            {"name": "info", "type": Object, "nullable": True, "default": None},
        ]
